...
const runExecutor = async (options, context) => {
    ...
    try {
        const cfg = JSON.parse(readFileSync(jsrJsonPath, 'utf-8'));
        ...
        logger.error(`Failed parsing jsr.json: ${e.message}`);
        return { success: false };
    }
    ...
    if (existsSync(tsconfigLibPath)) {
        try {
            ...
            if (decl !== true) {
                logger.error('tsconfig.lib.json must set compilerOptions.declaration: true for JSR libraries');
                return { success: false };
            }
        }
        catch (e) {
            logger.error(`Failed parsing tsconfig.lib.json: ${e.message}`);
            return { success: false };
        }
    }
    else {
