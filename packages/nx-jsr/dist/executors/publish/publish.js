...
const runExecutor = async (options, context) => {
    ...
    if (!existsSync(jsrJsonPath)) {
        logger.error(`jsr.json not found in ${absolutePackageRoot}. This is required for JSR publishing.`);
        return { success: false };
    }
    ...
    try {
        ...
        logger.info(`Successfully ${options.dryRun ? 'validated' : 'published'} package`);
        return { success: true };
    }
    catch (error) {
